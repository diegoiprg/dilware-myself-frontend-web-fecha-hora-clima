...
[![Version](https://img.shields.io/badge/version-v1.9.7-blue.svg)](./package.json)
...
- **Year Format Configuration**: Added new year format option in settings menu with "Completo (por defecto)" and "Abreviado (2 caracteres)" choices
- **Complete Date Display Defaults**: All date format settings now default to complete/full text display: "Sábado, 04 OCTUBRE 2025"
- **Year Format Logic**: Implemented year formatting that shows full year (2025) by default or abbreviated (25) based on user preference
- **Settings Menu Enhancement**: Added year format section to date configuration with clear default indicators
- **Date Display Integration**: Updated DateDisplay component to dynamically format year based on user settings
- **Context State Management**: Extended settings context to include yearFormat state and setter functions
...
- **Dark Mode Elegance Overhaul**: Complete redesign of dark mode for professional and elegant appearance matching light mode quality
- **Icon Background Removal**: Eliminated colored backgrounds from all weather icons in dark mode for cleaner, more modern look
- **Pearlescent Color Palette**: Replaced pure white with sophisticated pearlescent tones (95% brightness) for better visual comfort
- **Typography Enhancement**: Introduced Inter font family for dark mode with improved readability and modern aesthetics
- **Gradient Backgrounds**: Added subtle linear gradients to dark backgrounds for depth and visual interest
- **Enhanced Glassmorphism**: Improved backdrop blur effects with refined opacity, borders, and shadow effects for premium feel
- **Cross-browser Compatibility**: Added webkit prefixes for Safari compatibility with backdrop-filter effects
- **Font Hierarchy**: Implemented conditional font loading - Inter for dark mode, Orbitron for light mode headlines
...
- **Header Streamlined**: Single row header with version on left and hamburger menu settings on right, eliminating redundant version row for cleaner layout
- **Enhanced UV Index Indicator**: UV index now displays descriptive text (Bajo/Moderado/Alto/Muy alto/Extremo) with traffic light color coding matching air quality system
- **Removed Version Check System**: Completely eliminated non-functional version checking functionality that never worked properly, simplifying the codebase
- **Hamburger Menu Icon**: Replaced settings gear icon with universally recognized hamburger menu icon for better UX consistency
- **Layout Optimization**: Reduced grid from 5 rows to 4 rows in portrait mode by consolidating header elements
